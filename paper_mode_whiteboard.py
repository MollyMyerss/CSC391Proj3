...
def save_canvas_screenshot(canvas):
    os.makedirs("paper_mode_screenshots", exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"paper_mode_screenshots/image_{timestamp}.png"
    cv2.imwrite(filename, canvas)
    print(f"[screenshot] Saved canvas to {filename}")
    ...
    while True:
        ...
        if marker_center is None:
            ...
            if H is not None and not pen_paused:
                # Only draw if pen is NOT paused
                ...
                # Pen paused -> do NOT draw, and break the stroke
                prev_canvas_pt = None
                smoothed_canvas_pt = None
        ...
        # Show current color + pen mode
        mode_text = "PAUSED" if pen_paused else "DRAWING"
        cv2.putText(
            frame_with_drawing,
            f"Color: {color_name_display}  |  Mode: {mode_text}",
            (10, 30),
        ...
        if key == ord('q') or key == 27:
            ...
            # Screenshot of the RAW canvas (right panel)
            save_canvas_screenshot(canvas)
        elif key == 32:  # SPACE
            pen_paused = not pen_paused
            # Break the stroke so lines don't connect across pen up/down
            prev_canvas_pt = None
            smoothed_canvas_pt = None
